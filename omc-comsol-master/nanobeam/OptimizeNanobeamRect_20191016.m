...
function pp = OptimizeNanobeamRect_20191016(datLoc)
clear global P
...
P.freq = 7.7e9;                           % target mechanical frequency
...
P.LStats.ymin = 0;
P.LStats.ymax = 60e-9;
P.LStats.zmin = 0;   % z-coords relative to center of beam
P.LStats.zmax = P.th/2-50e-9;
...
optimds.fitStr = ['min(abs(omds.cpl.gMax.*omds.cpl.optWvl./299792458),2.0e5/1.96e14)./(2.0e5/1.96e14)',...
            ...
            '.*real(omds.cpl.SiV_111.full.LSiVAperMean)./1e6'];
optimds.parVec = '[P.th*1e6,P.w*1e6,P.a*1e6,P.hx/P.a,P.hy/P.w,P.oblong/10,P.maxdef]';

%% define optimization parameter space
sampbnds = [0.05,0.95]; % limits in which starting point is to be found
                      % e.g. [0.1,0.9] samples 10-90% of bounds
xnom = eval(optimds.parVec);    %nominal params
xmin = [0.2, 0.7, 0.40, 0.20, 0.40, 0.12, 0.05];
xmax = [0.3, 1.3, 0.65, 0.55, 0.80, 0.75, 0.30];
    ...
    if randomStart 
        ...
        while ~goodStart
            ...
            try 
                ...
                % plotting & saving
                PS.plotgeom = 1;                         % 1 to plot the geometry
                ...
                PS.mbevenz = 1;                          % 1 to find even mechanical mode about z
                ...
                PS.freq = bds0.sym.midGap(1);
                
                % solve nanobeam modes
...
% stop optimization if next iteration changes minimally from previous
% iteration
% usually 1%; for longer param vectors, set 5%
if(max(abs(x-y)./y)<0.025)
