% script to run mechanical bandstructure simulations for cross-hatch unit cell
...
P.celltype = 'solid';       % type of unit cell: 'solid' for solid cross (inner block + small fins), 
                            % 'hollow' for block with cross-shaped hole (block - cross)

P.a = 100e-9;              % lattice constant (along x)
P.w = 100e-9;              % unit cell width (along y)
P.th = 80e-9;              % unit cell thickness (along z)
P.cx = 70e-9;             % height (along x) of cross (for celltype = 'hollow')
                            % or of inner block (for celltype = 'solid')
P.cy = 70e-9;             % width (along y) of cross (for celltype = 'hollow')
                            % or of inner block (for celltype = 'solid')
P.tx = 20e-9;              % height (along x) of each leg in cross (for celltype = 'hollow')
                            % or of outer fins (for celltype = 'solid')
P.ty = 20e-9;              % width (along y) of each leg in cross (for celltype = 'hollow')
                            % or of outer fins (for celltype = 'solid')

P.filRad = 7e-9;           % fillet radius to round inner vertices of unit cell
...
%% Run
datLoc = 'C:\Users\mhaas\Documents\GitHub\device_design\omc-comsol-master\mechbands2D\';
bds = solveBands2D(P,datLoc);
