...
function ds = RunOpticalBands(P,ofem,datLoc)
...
% end
fBase = P.fileBase;
if ~exist([datLoc,fBase],'dir') && ofem.saveplots
    mkdir([datLoc,fBase])
end
...
if isfield(P,'celltype') && strcmp(P.celltype,'blockTet')
    ...
    [model,P] = DrawOptUnitCell(model,P,ofem);
end
...
disp('EM Waves Frequency Domain added - boundary conditions done');
...
%% Add Mesh
fem = obfem;
fem.mesh_quality = ofem.meshSize;
