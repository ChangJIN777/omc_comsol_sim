...
function [model,P] = DrawOptUnitCell(model,P,ofem)
...
cyl_cut_wp = ucellgeom.feature.create('cyl_cut_wp', 'WorkPlane');
cyl_cut_wp.set('planetype', 'quick').set('quickplane', 'xz');
cyl_cut = cyl_cut_wp.geom.feature.create('cyl_cut','Rectangle');
cyl_cut.set('type','solid').set('pos',[0,0]).set('size',[P.beamLen,airrad]);

air_cyl = ucellgeom.feature.create('air_cyl', 'Revolve');
...
ucellHolesAir = ucellgeom.feature.create('ucellHolesAir', 'Compose');
ucellHolesAir.selection('input').set(finBeamTag);
ucellHolesAir.selection('input').set('air_cyl');
ucellHolesAir.set('formula', [finBeamTag,' + air_cyl']);
ucellgeom.run;
displayCylStr = ', air cylinder';
finBeamTag = 'ucellHolesAir';

% % track max dimensions for selections
% totLen = beamLen;
% maxWid = max(maxWid,2*airrad);
% maxThi = max(maxThi,2*airrad);


%% Symmetry in z
if strcmp(P.xsect,'rect') && abs(ofem.obevenz)
    symZth = airrad;
    symW = airrad;
    ...
    ZsymSel.set('ymin', -delta).set('ymax', airrad+delta);
...
XsymSel.set('ymin', -delta).set('ymax', airrad+delta);
XsymSel.set('zmin', -airrad*(1-evenz)-delta).set('zmax', airrad+delta);
...
XendSel.set('ymin', -delta).set('ymax', airrad+delta);
XsymSel.set('zmin', -airrad*(1-evenz)-delta).set('zmax', airrad+delta);
...
YsymSel.set('zmin', -airrad*(1-evenz)-delta).set('zmax', airrad+delta);
...
cylCurvSel = ucellgeom.create('cylCurvSel', 'BoxSelection');
cylCurvSel.set('xmin', delta).set('xmax', 2*delta);
cylCurvSel.set('ymin', airrad-delta).set('ymax', airrad+delta);
cylCurvSel.set('zmin', delta).set('zmax', 2*delta);
cylCurvSel.set('entitydim', 2).set('condition', 'intersects');
ucellgeom.runCurrent;
inds = model.selection([ucellname,'_cylCurvSel']).inputEntities();
P.bndSel.cylCurv = inds';

% cyl bottom curved plane
cylCurvSel = ucellgeom.create('cylCurvSel2', 'BoxSelection');
cylCurvSel.set('xmin', delta).set('xmax', 2*delta);
cylCurvSel.set('ymin', airrad-delta).set('ymax', airrad+delta);
cylCurvSel.set('zmin', -2*delta).set('zmax', -delta);
cylCurvSel.set('entitydim', 2).set('condition', 'intersects');
ucellgeom.runCurrent;
inds = model.selection([ucellname,'_cylCurvSel2']).inputEntities();
...
%     cylCurv2Sel.set('ymin', airrad/sqrt(2)-10*delta).set('ymax', airrad/sqrt(2)+10*delta);
%     cylCurv2Sel.set('zmin', -airrad/sqrt(2)-10*delta).set('zmax', -airrad/sqrt(2)+10*delta);
...
% unit cell: define box slightly larger and fully containing full beam volume
ucellSel = ucellgeom.create('ucellSel', 'BoxSelection');
ucellSel.set('xmin', -delta).set('xmax', nperiod*a+delta);
ucellSel.set('ymin', -delta).set('ymax', w/2 + delta);
ucellSel.set('zmin', -th/2-delta).set('zmax', th/2 + delta);
ucellSel.set('entitydim', 3).set('condition', 'inside');
ucellgeom.runCurrent;
inds = model.selection([ucellname,'_ucellSel']).inputEntities();
P.domSel.ucell = inds';

% air cylinder: define box intersecting air cyl outside unit cell
cylSel = ucellgeom.create('cylSel', 'BoxSelection');
cylSel.set('xmin', -delta).set('xmax', nperiod*a+delta);
cylSel.set('ymin', w/2 + delta).set('ymax', w/2 + 2*delta);
cylSel.set('zmin', -th/2 - delta).set('zmax', th/2 + delta);
cylSel.set('entitydim', 3).set('condition', 'intersects');
ucellgeom.runCurrent;
inds = model.selection([ucellname,'_cylSel']).inputEntities();
P.domSel.cyl = inds';
