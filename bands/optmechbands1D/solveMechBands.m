...
function ds = solveMechBands(P,mfem)
...
if ~isfield(P,'fileBase')
    fBase = ['mech_a',num2str(P.a*1e9,'%.0f'),'nm_',...
...
if isempty(dir([datLoc,fBase,'*_bds.mat']))
    ...
    if strcmp(P.xsect,'rect')
        disp('in Y and Z (rect x-sect)');
        mfem.mbevenz = 1;
    end
    sym = RunMechBands(P,mfem,datLoc);
    [sym.midGap,sym.gapSize] = findGaps(sym);   %find gaps
    
    %Saving the gap properties for later use
    ds.midGap = sym.midGap(1);
    ds.gapSize = sym.gapSize(1);
    ...
    if mfem.solveasym == 1
        ...
        asym = RunMechBands(P,mfem,datLoc);
        [asym.midGap,asym.gapSize] = findGaps(asym);    %find gaps
        
        if strcmp(P.xsect,'rect')
            disp('Solving with Y anti-symmetric and Z symmetric boundary condition');
            mfem.mbeveny = -1;
            mfem.mbevenz = 1;
            asymYsymZ = RunMechBands(P,mfem,datLoc);
            [asymYsymZ.midGap,asymYsymZ.gapSize] = findGaps(asymYsymZ);    %find gaps
            
            disp('Solving with Y symmetric and Z antisymmetric boundary condition');
            mfem.mbeveny = 1;
            mfem.mbevenz = -1;
            symYasymZ = RunMechBands(P,mfem,datLoc);
...
if mfem.savebndplot == 1
    ...
    if mfem.completeBandGaps && ~isempty(complete.midGap)
        ...
        if mfem.solveasym == 1 && strcmp(P.xsect,'tri')
