...
Ported unchanged from python-scripts/src/bandgap.py -- this module is
backend- and dimensionality-agnostic: every solver returns frequencies as an
array of shape [n_k, n_bands] (Hz), and these helpers just find gaps between
consecutive sorted bands. Works identically for the 1D nanobeam project and
this 2D phononic-crystal project.
...
Two notions of gap:
  ...
  - symmetry gap : pass only the bands of one symmetry family (filter upstream
                   -- e.g. one z-parity, evenz or oddz, for this 2D project),
...
def largest_gap(freqs: np.ndarray, f_window: Optional[tuple] = None,
    ...
    nan_safe : if True, use nanmax/nanmin so NaN entries are ignored. Use this
        ...
        one z-parity kept, others set to NaN). Both band edges must be finite
        for a gap to be accepted.
...
def all_gaps(freqs: np.ndarray, f_window: Optional[tuple] = None,
    ...
    Same gap criterion as `largest_gap`, but returns a Gap for EVERY
    qualifying (i, i+1) pair, sorted by normalized_gap descending.
    """
    freqs = np.asarray(freqs, dtype=float)
    if freqs.ndim != 2:
        raise ValueError("freqs must be [n_k, n_bands]")
