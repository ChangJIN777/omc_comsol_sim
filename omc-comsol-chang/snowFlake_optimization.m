...
function [fullMidBand, fullGapSize] = run_snowFlake_fixed_a(P,w,r)    
    %% unit cell params 
    P.xsect = 'rect'; 
    P.beamMat = 'diamond';                  % beam material name
    P.celltype = 'snowflake';                   % specify the cell type
    P.unitcell = 'hexagonal';                  % specify the shape of the unit cell
    P.a = 300e-9;              % lattice constant 
    P.w = w;              % unit cell width (along x)
    P.r = r;              % unit cell height (along y)
    P.th = 120e-9;             % height (along x) of cross (for celltype = 'hollow')
                                % or of inner block (for celltype = 'solid')
    ...
    %% run the simluation and save the data
    datLoc = '.\test\snowflake\092024_sweep2\';
    P.datLoc = datLoc;
    bds = solveBands(P);
...
function fitness = runOptimization_fixedLattice(params)
    P.celltype = 'snowflake';                   % specify the cell type
    a = 300e-9;
    w = params(1);
    r = params(2);
    %% account for any fabrication intolerance 
    smallestFeature = (a/2-r)-(w/2)/sqrt(3);
    if smallestFeature < 1e-10
        fitness = 0;
    else
        [fullMidBand, fullGapSize] = run_snowFlake_fixed_a(P,w,r);
        fitness_list = calFitnessSnow(fullMidBand,fullGapSize);
        fitness = min(fitness_list);
    end
end