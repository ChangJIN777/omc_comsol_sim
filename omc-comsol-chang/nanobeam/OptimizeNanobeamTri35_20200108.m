...
function pp = OptimizeNanobeamTri35_20200108(datLoc)
clear global P
...
P.w = 1018e-9;                        % beam width
P.theta = 35;                           % etch angle in degrees (no effect for rect cross section)
P.th = P.w/(2*tan(P.theta*pi/180));     % beam thickness
P.hx = 290e-9;                        % nominal hole height (along x-axis)
P.hy = 611e-9;                        % nominal hole width (along y-axis)
...
P.maxdef = 0.0949;                     % defect percentage
P.oblong = 2.9172;                      % oblong parameter (zero if holes are not changed)
...
% boundary conditions (numeric value settings for FDTD): 
% 1 = PML, 2 = metal, 3 = periodic
% 4 = symmetric, 5 = anti-symmetric, 6 = Bloch, 7 = PMC
% tri x-sect TE: yminBC = 5, ymaxBC = 1 (if not periodic)
% rect x-sect TE: yminBC = 5, zminBC = 4, ymaxBC and zmaxBC = 1 (if not periodic)
P.xminBC = 1;
P.xmaxBC = 1;
P.yminBC = 5;
P.ymaxBC = 1;
...
file.path = 'C:\Users\CCHIA\Documents\GitHub\OMC\NanobeamFDTD\FDTD scripts\';
...
xmin = [0.90, 0.50, 0.30, 0.50, 0.10, 0.05];
xmax = [1.10, 0.60, 0.65, 0.80, 0.35, 0.30];
    ...
    if randomStart 
        ...
        while ~goodStart
            ...
            try 
                ...
                % find bandgap closest to flapping mode frequency
                bds0 = solveBands(PS,datLoc);
                [~,gapIdx] = max(bds0.sym.gapSize./abs(bds0.sym.midGap-5e9));
%                 disp(bds0.sym.gapSize./abs(bds0.sym.midGap-5e9))
                gapFlap = gapIdx(1);
%                 disp(gapFlap)
...
addpath('C:\Users\CCHIA\Documents\GitHub\OMC\UnitCell1DFEM\');
addpath('C:\Users\CCHIA\Documents\GitHub\OMC\NanobeamFDTD\');
close all;
...
try
    ...
    [~,gapIdx] = max(bds.sym.gapSize./abs(bds.sym.midGap-5e9));
%     disp(bds.sym.gapSize./abs(bds.sym.midGap-5e9))
    gapFlap = gapIdx(1);
%     disp(gapFlap)
    if bds.sym.gapSize(gapFlap) > 1e9
        ...
        Pf = omds.P;
        Pf.lambda = omds.cpl.optWvl;
        ods = runNanobeamCavity(Pf,file,datLoc);
        omds.ods = ods;
        ...
        disp(bds.sym.midGap./1e9)
        disp(bds.sym.gapSize./1e9)
        error('no band gaps > 1 GHz')
    end
