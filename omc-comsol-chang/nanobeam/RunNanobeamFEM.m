...
%     disp(mModes)
    ...
    if P.calcS
        ...
        % pre-select modes to calc coupling over if not calc gOM
        if ~P.calcG && P.calcS && length(mModes) > 1
            [~,~,dsSiV] = CalcStrCplSiV(ds,model,mModes,P.xSlc,P.ySlc,P.zSlc,P.zSiV,[],0);
            
            % filter by coupling then by frequency
            mFilterL = find(dsSiV.LSiVMaxMode>0.7e6);
            if isempty(mFilterL)
                [~,mFilterL0] = sort(dsSiV.LSiVMaxMode,'descend');
                mFilterL = mFilterL0(1:2);
                disp('no modes with spin-phonon coupling > 0.7 MHz')
            end
            mModes = mFilterL;
            disp(['modes ',num2str(mModes),' selected for spin-phonon coupling'])
            clear dsSiV
        end
        ...
        yposHalf = [0:interpResY:(max(ds.P.w_hole))/2,(max(ds.P.w_hole))/2];
        ...
        [ds,model,~] = CalcStrCplSiV(ds,model,mModes,xpos,ypos,zpos,P.zSiV,LStats,1);
    ...
    if P.calcS
        LSiVMax = ds.cpl.SiV.LSiVMaxMode; 
        if P.calcG
    ...
    if P.plotStrCpl
        ...
        for mIdx = 1:length(mModes)
            for oi = 1:length(P.zSiV)
                maxSec = ds.cpl.SiV.LSiVMaxSec(1,1,mIdx,oi);
                maxPos = ds.cpl.SiV.LSiVMaxPos(1:3,1,mIdx,oi,maxSec);
                pSlc(1:3,mIdx,oi) = maxPos(1:3);
            end
        end
        PlotStrCplSiVXY(ds,model,mModes,pSlc,P.zSiV,datLoc)
        PlotStrCplSiVYZ(ds,model,mModes,pSlc,P.zSiV,datLoc)
        ...
        if isfield(P,'ySlc') && isfield(P,'zSlc') 
            ...
            pltXY = PlotStrCplSiVXY(ds,model,mModes,pSlc,P.zSiV,datLoc);
            pltYZ = PlotStrCplSiVYZ(ds,model,mModes,pSlc,P.zSiV,datLoc);
            ds.cpl.SiV.LXYSlc = pltXY.LSiVSlc;
            ds.cpl.SiV.LYZSlc = pltYZ.LSiVSlc;
            ds.cpl.SiV.LXYModes = pltXY.LSiVSlcModes;
            ds.cpl.SiV.LYZModes = pltYZ.LSiVSlcModes;
        end
    end
