...
function [P] = CreateNanobeamGeomSnowflake(P)
...
MN_right = P.MN_right;
TN = P.TN; 
% the parameters for the defect cells 
...
wo_defect = generate_defect_region(wo,wo_ctr,taperingNum);
wi_defect = generate_defect_region(wi,wi_ctr,taperingNum);

% generate the mirror cells 
a_MN_left = ones(1,MN_left)*a;
ho_MN_left = ones(1,MN_left)*ho;
hi_MN_left = ones(1,MN_left)*hi;
wo_MN_left = ones(1,MN_left)*wo;
wi_MN_left = ones(1,MN_left)*wi;

a_MN_right = ones(1,MN_right)*a;
ho_MN_right = ones(1,MN_right)*ho;
hi_MN_right = ones(1,MN_right)*hi;
wo_MN_right = ones(1,MN_right)*wo;
wi_MN_right = ones(1,MN_right)*wi;

a_list = [a_MN_left,a_defect,a_MN_right];
ho_list = [ho_MN_left,ho_defect,ho_MN_right];
hi_list = [hi_MN_left,hi_defect,hi_MN_right];
wo_list = [wo_MN_left,wo_defect,wo_MN_right];
wi_list = [wi_MN_left,wi_defect,wi_MN_right];
...
P.geom = [wo_list' wi_list' ho_list' hi_list' a_list'];
...
function a_taper = cubic_tapering(a,amin,taperingNum)
    %% this function generate a list of numbers that follows the cubic tapering scheme from the value a to amin
    a_taper = zeros(1,taperingNum);
    d = 1-(amin/a);
    for i=1:taperingNum 
        a_taper(i) = a*(1-d*(2*((i/taperingNum)^3) - 3*((i/taperingNum)^2)+ 1));
    end
end
function a_defect = generate_defect_region(a,amin,taperingNum)
    %% this function generate a list of numbers that parameterize a defect region of a cavity 
    a_left_region = cubic_tapering(a,amin,taperingNum);
    a_right_region = cubic_tapering(a,amin,taperingNum);
    a_left_region = flip(a_left_region);
    a_defect = [a_left_region,amin,a_right_region];
end