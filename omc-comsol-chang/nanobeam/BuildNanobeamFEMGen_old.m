...
function [model,P] = BuildNanobeamFEMGen(model,P)
...
if isfield(P,'celltype') && strcmp(P.celltype,'blockTet')
    ...
    %     hole = holes.geom.feature.create(['hole_' num2str(k)], 'Ellipse');
        if strcmp(P.holeType,'tri') || strcmp(P.holeType,'anvil')
            ...
            poly.set('table', [xpos(k)+hx(k)/2 ypos(k);xpos(k) hy(k)/2;xpos(k)-hx(k)/2 ypos(k);xpos(k) -hy(k)/2]);
            if strcmp(P.holeType,'anvil')
                filID = ['Filet_1_',num2str(k)];
                filOne = beamWP.geom.create(filID, 'Fillet');
        ...
        elseif strcmp(P.holeType,'hole') 
            ...
            curveOne.set('parmax', -hx(k)/2);
            curveOne.set('coord', {'s' [num2str(-hy(k)/(a(k)-hx(k))^2,8),'*(s+',num2str(a(k)/2,8),')^2 + ',num2str(wid/2,8)]});
            curveTwoName = ['curveTwo',num2str(k)];
            curveTwo = beamWP.geom.create(curveTwoName, 'ParametricCurve');
            curveTwo.set('parmin', -hx(k)/2);
            curveTwo.set('parmax', a(k)/2-hx(k));
            curveTwo.set('coord', {'s' [num2str((wid-hy(k))/2,8),'+', num2str(hy(k)/(a(k)-hx(k))^2,8),'*(s+',num2str(hx(k)-a(k)/2,8),')^2']});

            LineBotName = ['BotLine',num2str(k)];
            lineBot = beamWP.geom.create(LineBotName, 'LineSegment');
            lineBot.set('specify1', 'coord');
            lineBot.set('coord1', [(a(k)/2-hx(k)) (wid-hy(k))/2]);
            lineBot.set('specify2', 'coord');
            lineBot.set('coord2', [(hx(k)-a(k)/2) (wid-hy(k))/2]);

            curveThreeName = ['curveThree',num2str(k)];
            curveThree = beamWP.geom.create(curveThreeName, 'ParametricCurve');
            curveThree.set('parmin', hx(k)-a(k)/2);
            curveThree.set('parmax', hx(k)/2);
            curveThree.set('coord', {'s' [num2str( (wid-hy(k))/2,8),'+', num2str(hy(k)/(a(k)-hx(k))^2,8),'*(s+',num2str(-hx(k)+a(k)/2,8),')^2']});

            curveFourName = ['curveFour',num2str(k)];
            curveFour = beamWP.geom.create(curveFourName, 'ParametricCurve');
            curveFour.set('parmin', hx(k)/2);
            curveFour.set('parmax', a(k)/2);
            curveFour.set('coord', {'s' [num2str(-hy(k)/(a(k)-hx(k))^2,8),'*(s-',num2str(a(k)/2,8),')^2+',num2str(wid/2,8)]});

            LineTopName = ['TopLine',num2str(k)];
            lineTop = beamWP.geom.create(LineTopName, 'LineSegment');
            lineTop.set('specify1', 'coord');
            lineTop.set('coord1', [-a(k)/2 wid/2]);
            lineTop.set('specify2', 'coord');
            lineTop.set('coord2', [a(k)/2 wid/2]);

            convertName = ['csol',num2str(k)];
            convert = beamWP.geom.create(convertName, 'ConvertToSolid');
            ...
            holeID = ['mov',num2str(k)];
            mov = beamWP.geom.create(holeID, 'Move');
            mov.setIndex('displx', xpos(k), 0);
