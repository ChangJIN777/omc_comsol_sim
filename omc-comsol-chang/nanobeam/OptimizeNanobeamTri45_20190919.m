...
function pp = OptimizeNanobeamTri45_20190919(datLoc)
clear global P
...
maxSPtoRun = 15;     % no. of starting points to run for; 0 to run indefinitely
...
optimds.fitStr = ['min(abs(omds.cpl.gMax.*omds.cpl.optWvl./299792458),3.0e5/1.96e14)./(3.0e5/1.96e14)',...
            '.*min([omds.cpl.Q,5e6])./5e6',...
            '.*real(omds.cpl.SiV_111.full.LSiVAperMax)./1e6'];
optimds.parVec = '[P.oblong/10,P.maxdef]';
