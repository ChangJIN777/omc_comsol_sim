...
%
%   Sweeps P.oblong x P.maxdef on an (ndefectAspectRatio x nMaxdef) grid using the
...
%
%   Returns:
%     oVec        – (ndefectAspectRatio x 1) oblong sweep vector
%     mVec        – (nMaxdef x 1) maxdef sweep vector
%     fitnessMap  – (ndefectAspectRatio x nMaxdef) fitness at each grid point
...
OPT.rootLoc = [fullfile('.','test','1D_OMC_hole',['sweep_oblongMaxdef_trial3_',currentDate]),filesep];
...
%% ===================== BUILD SWEEP GRIDS =====================
oVec = linspace(OPT.defectAspectRatio_min, OPT.defectAspectRatio_max, OPT.ndefectAspectRatio)';
mVec = linspace(OPT.maxdef_min, OPT.maxdef_max, OPT.nMaxdef)';

fitnessMap = nan(OPT.ndefectAspectRatio, OPT.nMaxdef);
wM_map     = nan(OPT.ndefectAspectRatio, OPT.nMaxdef);
gOM_map    = nan(OPT.ndefectAspectRatio, OPT.nMaxdef);
LSiV_map   = nan(OPT.ndefectAspectRatio, OPT.nMaxdef);
Qmech_map  = nan(OPT.ndefectAspectRatio, OPT.nMaxdef);
Qopt_map   = nan(OPT.ndefectAspectRatio, OPT.nMaxdef);
lambda_map = nan(OPT.ndefectAspectRatio, OPT.nMaxdef);  % optical wavelength (nm)
...
evalCount = 0;
nTotal    = OPT.ndefectAspectRatio * OPT.nMaxdef;

%% ===================== 2-D SWEEP =====================
for io = 1:OPT.ndefectAspectRatio
    for im = 1:OPT.nMaxdef
        evalCount = evalCount + 1;
        defectAspectRatio_i = oVec(io);
        maxdef_i = mVec(im);

        Pe = P;
        
        % calculating oblong from the defect aspect ratio
...
[ioBest, imBest]      = ind2sub([OPT.ndefectAspectRatio, OPT.nMaxdef], linIdx);
...
fprintf('Grid size     = %d x %d  (%d total evals)\n', OPT.ndefectAspectRatio, OPT.nMaxdef, nTotal);
