...
% symmetric BCs
symBCs_z = smech.create('symBCs_z', 'SymmetrySolid', 2);
symBCs_z.label('Symmetric BC z');
symBCs_y = smech.create('symBCs_y', 'SymmetrySolid', 2);
symBCs_y.label('Symmetric BC y');
bnds.sym_inds = [];
if meveny == 1
    symBCs_y.selection.named('slab_yboundaries_bnd');
end
if mevenz == 1 && strcmp(P.xsect,'rect')
   symBCs_z.selection.named('slab_zboundaries_bnd');
end
if mevenz~=1 
    symBCs_z.active(false);
end
if meveny~=1 
    symBCs_y.active(false);
end

% anti-symmetric BCs
asymBCs_z = smech.create('asymBCs_z', 'Antisymmetry', 2);
asymBCs_z.label('Anti-symmetric BC z');
asymBCs_y = smech.create('asymBCs_y', 'Antisymmetry', 2);
asymBCs_y.label('Anti-symmetric BC y');
bnds.asym_inds = [];
% if mevenx == -1
%     bnds.asym_inds = [bnds.asym_inds,P.bndSel.beamXsym];
% end
if meveny == -1
    asymBCs_y.selection.named('slab_yboundaries_bnd');
end
if mevenz == -1 && strcmp(P.xsect,'rect')
    asymBCs_z.selection.named('slab_zboundaries_bnd');
end
if meveny~=-1
    asymBCs_y.active(false);
end
if mevenz~=-1
    asymBCs_z.active(false);
end
