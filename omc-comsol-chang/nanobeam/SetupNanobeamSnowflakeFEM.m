...
if mevenx==1
    bnds.fixed_inds = [bnds.fixed_inds,P.bndSel.Xsym_r];    
end
...
if mevenx == 1
    ...
    bnds.sym_inds= [bnds.sym_inds,P.bndSel.Xsym_l];
end
if meveny == 1
    %set BC for faces in yz-plane containing point (0,0,0)
...
if mevenx == -1
    %set BC for faces in yz-plane containing point (0,0,0)
    bnds.asym_inds= [bnds.asym_inds,P.bndSel.Xsym_l];
end
if meveny == -1
    %set BC for faces in yz-plane containing point (0,0,0)
...
if P.solveOpt
    ...
    if oevenx == 1
        %set BC for faces in yz-plane containing point (0,0,0)
        bnds.pmc_inds = [bnds.pmc_inds,P.bndSel.Xsym_l];
    end
    if oeveny == 1
    ...
    bnds.scat_inds = [P.bndSel.cylCurv,P.bndSel.Xsym_r];

    if P.solveOpt && ~isempty(bnds.scat_inds)
    ...
    PECbnd = emw.create('PECbnd', 'PerfectElectricConductor', 2);
    PECbnd.label('Perfect Electric Conductor');
    bnds.pec_inds = [];
    if oevenx == -1
        %set BC for faces in yz-plane containing point (0,0,0)
        bnds.pec_inds = [bnds.pec_inds,P.bndSel.Xsym_l];
    end
    if oeveny == -1
        %set BC for faces in xz-plane containing point (0,0,0)
    ...
    if ~isempty(bnds.pec_inds)
        PECbnd.selection.set(bnds.pec_inds);
