...
func = @(params) runSnowflakeFEM_optOnly(params);
...
function minFitness = runSnowflakeFEM_optOnly(params)    
    ...
    %% specify simulation/calculation/plot/save options
    P.solveMech = 0;                        % 1 to solve for mechanics
    P.solveOpt = 1;                         % 1 to solve for optics
    P.nbeam = 2.4028;        % refractive index of the dielectric material
    P.calcG = 1*(P.solveMech && P.solveOpt);% 1 to calculate optomechanical coupling
    P.calcS = 0*P.solveMech;                % 1 to calculate strain coupling
    ...
    ofem = ds.ofem;
%     cpl = ds.cpl;
    opt_Q = ofem.Q; % the Q factor of the optical mode 
    opt_lambda = ofem.optWvl; % the wavelength of the corresponding resonances 
    ...
    % calculate and find the minimum fitness
    fitness = calFitness_opt(opt_Q,opt_lambda);
    [minFitness,idx] = min(fitness);
       
    % record the data 
    data = [P.a,P.w,P.r,P.ho,P.hi,P.wo,P.wi,P.d,P.ho_ctr,P.hi_ctr,...
        P.wo_ctr,P.wi_ctr,opt_Q,opt_lambda,minFitness];
    writematrix(data,optFilename,'WriteMode','append');
...
function fitness = calFitness_opt(opt_QAll,opt_lambdaAll)
    wavelength_tolerance = 100e-9;
    fitness = -abs(opt_QAll.*exp(-((opt_lambdaAll-1550e-9)./(wavelength_tolerance)).^2));
end