...
a_hole = zeros(1,ntot);
hx_hole = zeros(1,ntot);
hy_hole = zeros(1,ntot);
...
if nPhM>0 && isfield(P, 'phonMir')
    for k = 1:nPhM
        ...
        hx_hole(nholes+nctr+k) = P.phonMir.hx;
        hy_hole(nholes+nctr+k) = P.phonMir.hy;
    end
end
...
elseif ~holeatctr && isfield(P,'cavlen') && P.cavlen > 0
    ...
    % half-beam
    xposHalf = cumsum([hx_hole(1)/2+cavlen/2,a_hole(2:end)]);
    ...
    hx_holeFull = [fliplr(hx_hole) hx_hole];
    hy_holeFull = [fliplr(hy_hole) hy_hole];
