% front panel script for nanobeam FEM simulations (example for tri cross-section, beam-hole geometry)
...
P.a = 580e-9;                           % nominal lattice constant
P.w = 929e-9;                           % beam width
P.theta = 35;                           % etch angle in degrees (no effect for rect cross section)
P.th = P.w/(2*tan(P.theta*pi/180));     % beam thickness
P.hx = 250e-9;                          % nominal hole height (along x-axis)
P.hy = 590e-9;                          % nominal hole width (along y-axis)

% hole params for symmetric cavity / right half of asymmetric cavity
P.nholes = 14;                          % # holes in 1/2 beam length
P.ndef = 7;                             % # of holes in 1/2 defect region
P.maxdef = 0.2;                         % defect percentage
P.oblong = 2.2;                         % oblong parameter (zero if holes are not changed)

% cavity taper params
P.holeatctr = 1;                        % 1/0 for hole/dielectric in middle
P.taperFunc = 'quadratic';              % linear/cubic/quadratic taper function to center hole in cavity
...
P.wgmTaper.hy_end = 202e-9;             % for endtype = 'custom': hole width of end hole
...
P.lambda = 1562e-9;                     % target optical wavelength
...
P.freq = 9e9;                           % target mechanical frequency
P.mneigs = 10;                          % # of eignevalues to find
P.mMesh = 2;                            % mesh quality for mechanical simulations
...
P.ySlc = 0;
% P.zSlc = P.th/2 - 50e-9; %flapping mode
P.zSlc = 70.4e-9; %swelling mode
...
% P.LStats.ymin = 0;
% P.LStats.ymax = 60e-9;
% P.LStats.zmin = P.th/2-50e-9;   % z-coords relative to center of beam
% P.LStats.zmax = P.th/2;

% for swelling mode
P.LStats.ymin = 0;
P.LStats.ymax = 100e-9;
P.LStats.zmin = -30e-9;   % z-coords relative to center of beam
P.LStats.zmax = 180e-9;
...
datLoc = 'D:\Files\SpinPhon\35oOpticaS\';  
