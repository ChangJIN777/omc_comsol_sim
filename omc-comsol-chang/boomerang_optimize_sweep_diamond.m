...
datLoc = [fullfile('.','test','diamond_ribUnitCell_optimize_run2',currentDate),filesep];
itrPath = [datLoc,...
...
function fitness = rib_optimize(params)
    ...
    P.w = params(2);              % unit cell width (along x)
    P.r = params(3);              % unit cell height (along y)
    P.th = params(4);             % height (along x) of cross (for celltype = 'hollow')
                                % or of inner block (for celltype = 'solid')
    P.h = params(5);
    P.d1 = params(6);
    P.d = sqrt(3)*P.a/2;    % the waveguide gap size 
    P.xsect = 'rect'; 
    ...
    P.mbevenz = 1;                          % 1 to find even mechanical mode about z
    P.freq = 15e9;                             % Mechanical target frequency - set to 0 for bandstructure simulations
    ...
    if min(P.a-P.r,min(P.d,P.w)) < 50e-9
    ...
    currentDate = datestr(now,'mmddyyyy');
    datLoc = [fullfile('.','test','diamond_ribUnitCell_optical_run2',currentDate),filesep];
    P.datLoc = datLoc;
    ...
    % mechanical band sim 
    datLoc = [fullfile('.','test','diamond_ribUnitCell_run2',currentDate),filesep];
    P.datLoc = datLoc;
    P.run_optical=0;
    ...
    gapBelow = bds_mechanical.midGap < 20e9;
    bds_mechanical.gapSize = bds_mechanical.gapSize.*gapBelow;
