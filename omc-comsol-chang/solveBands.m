...
if ~isfield(P,'fileBase')
    ...
    elseif strcmp(P.celltype,'cross_strip')
        % Strip / supercell of P.ncell square cross cells, built by
        % buildCrossStrip.m. Parameter names are the ones that builder
        % actually reads (P.a, P.h, P.w, P.th, P.r1, P.r2); P.ncell is
        % included so runs at different strip lengths cannot collide on the
        % same filename.
        fBase = ['cross_strip_','a_',num2str(P.a*1e9,'%.0f'),'nm_',...
            'h_',num2str(P.h*1e9,'%.0f'),'nm_',...
            'w_',num2str(P.w*1e9,'%.0f'),'nm_', ...
            'th_',num2str(P.th*1e9,'%.0f'),'nm_', ...
            'r1_',num2str(P.r1*1e9,'%.0f'),'nm_', ...
            'r2_',num2str(P.r2*1e9,'%.0f'),'nm_', ...
            'n_',num2str(P.ncell,'%.0f'),'_'];
        if isfield(P,'prefname')
            fBase = [P.prefname,'_',fBase];
        end
        P.fileBase = fBase;
    ...
    if P.savebndplot
        ...
        elseif strcmp(P.celltype,'cross')
            bandtitle = {['a = ',num2str(P.a*1e9,'%.0f'),'nm, ',...
                ...
                ['th = ',num2str(P.th*1e9,'%.0f'),'nm']};
        elseif strcmp(P.celltype,'cross_strip')
            % Uses P.w, not P.wc as the 'cross' branch above does. The two
            % builders disagree on the field name and buildCrossStrip reads
            % P.w; that pre-existing inconsistency is left untouched here.
            bandtitle = {['a = ',num2str(P.a*1e9,'%.0f'),'nm, ',...
                'h = ',num2str(P.h*1e9,'%.0f'),'nm, ',...
                'w = ',num2str(P.w*1e9,'%.0f'),'nm, ',...
                'r1 = ',num2str(P.r1*1e9,'%.0f'),'nm, ',...
