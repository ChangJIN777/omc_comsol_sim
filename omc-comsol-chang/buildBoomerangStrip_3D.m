...
ho = P.ho;
hi = P.hi;
d = P.d;        % center-hole offset
% the parameters for the center unit cell tabs (v2 style)
h = P.h;
d1 = P.d1;
% fillet radius
...
model.param.set('w',        sprintf('%.12g[m]', w));
model.param.set('r',        sprintf('%.12g[m]', r));
model.param.set('th',       sprintf('%.12g[m]', th));
model.param.set('wo',       sprintf('%.12g[m]', wo));
model.param.set('wi',       sprintf('%.12g[m]', wi));
model.param.set('ho',       sprintf('%.12g[m]', ho));
model.param.set('hi',       sprintf('%.12g[m]', hi));
model.param.set('h_tab',    sprintf('%.12g[m]', h));
model.param.set('d1',       sprintf('%.12g[m]', d1));
model.param.set('r1',       sprintf('%.12g[m]', r1));
model.param.set('r2',       sprintf('%.12g[m]', r2));
model.param.set('airDiskH', sprintf('%.12g[m]', P.airDiskH));
model.param.set('d',        sprintf('%.12g[m]', d));
model.param.set('delta',    sprintf('%.12g[m]', 10e-9));
...
ucellplane.set('size', {'a' 'sqrt(3)*(2+1/2)*a'});
hole_label_list = {};
...
% add fillets
addFillet(P,ucellWP.geom,1,hole1_center,'2*sel_w')
addFillet(P,ucellWP.geom,2,hole2_center,'sel_w')
addFillet(P,ucellWP.geom,3,hole3_center,'sel_w')
addFillet(P,ucellWP.geom,4,hole4_center,'sel_w')
addFillet(P,ucellWP.geom,5,hole5_center,'sel_w')
addFillet(P,ucellWP.geom,6,hole6_center,'sel_w')
addFillet(P,ucellWP.geom,7,hole7_center,'sel_w')
addFillet(P,ucellWP.geom,8,hole8_center,'sel_w')
addFillet(P,ucellWP.geom,9,hole9_center,'sel_w')
addFillet(P,ucellWP.geom,10,hole10_center,'sel_w')
addFillet(P,ucellWP.geom,11,hole11_center,'sel_w')
addFillet(P,ucellWP.geom,12,hole12_center,'sel_w')
addFillet(P,ucellWP.geom,13,hole13_center,'sel_w')
...
if abs(P.mbevenz)
    ...
    symZPlane.set('table', {'-a/2' '0'; 'a/2' '0'; 'a/2' 'sqrt(3)*(4+1/2)*a'; '-a/2' 'sqrt(3)*(4+1/2)*a'; '-a/2' '0'});
    ...
    ZsymSel.set('ymin', '-delta').set('ymax', 'sqrt(3)*(4+1/2)*a+delta');
...
function hole_rec3_label_list = add_BoomerangHole(ucellWP,hole_center,P,holeIndex)
    ...
    hole_rec_3.set('pos', {[hole_center{1} '+sqrt(3)*r/4'], [hole_center{2} '-r/4']});
...
function hole_rec3_label_list = add_BoomerangHole_center_v2(ucellWP,hole_center,P,holeIndex)
    ...
    hole_rec_4.set('pos', {[hole_center{1} '+sqrt(3)*r/2+w/4'], [hole_center{2} '-r/2+sqrt(3)*w/4']});
    hole_rec_4.set('base', 'corner');
    hole_rec_4.set('rot', 180);
    hole_rec_4.set('size', {'d1' 'h_tab'});
    hole_rec_5 = ucellWP.geom.feature.create(hole_rec5_label, 'Rectangle');
    hole_rec_5.set('pos', {[hole_center{1} '-sqrt(3)*r/2-w/4'], [hole_center{2} '-r/2+sqrt(3)*w/4-h_tab']});
    hole_rec_5.set('base', 'corner');
    hole_rec_5.set('size', {'d1' 'h_tab'});
...
function addFillet(P,ucellgeom,hole_indx,hole_pos,selection_width)
    ...
    disksel2.set('r', sprintf('r+(%s)', selection_width));
    disksel2.set('rin', sprintf('r-(%s)', selection_width));
...
function addFillet_center(P,ucellgeom,hole_indx,hole_pos,selection_width)
    ...
    disksel1.set('posx', hole_pos{1});
    disksel1.set('posy', hole_pos{2});
    disksel1.set('r', sprintf('sqrt(hi^2+(wi/2)^2)+(%s)', selection_width));
    disksel1.set('rin', sprintf('wi/2-(%s)', selection_width));
    disksel1.set('condition', 'allvertices');
    fil1 = ucellgeom.feature.create(fillet1_label, 'Fillet');
    fil1.set('radius', 'r1');
    fil1.selection('point').named(disksel1_label);
    ...
    disksel2.set('posy', hole_pos{2});
    disksel2.set('posx', hole_pos{1});
    disksel2.set('r', sprintf('sqrt(ho^2+(wo/2)^2)+(%s)', selection_width));
    disksel2.set('rin', sprintf('wo/2-(%s)', selection_width));
    disksel2.set('condition', 'allvertices');
    fil2 = ucellgeom.feature.create(fillet2_label, 'Fillet');
    fil2.set('radius', 'r2');
    fil2.selection('point').named(disksel2_label);
